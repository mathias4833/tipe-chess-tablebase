...
|(ShortCastling, true) -> {  (* Petit roque blanc *)
  ...
  |(Chessmove(p, n_from, n_to), _) -> (
    ...
    |(P, true) -> {tempboard with wpawns = Bitboard.set_nth tempboard.wpawns n_to}
    |(B, true) -> {tempboard with wbishops = Bitboard.set_nth tempboard.wbishops n_to}
    |(N, true) -> {tempboard with wknights = Bitboard.set_nth tempboard.wknights n_to}
    |(R, true) -> {tempboard with wrooks = Bitboard.set_nth tempboard.wrooks n_to}
    |(Q, true) -> {tempboard with wqueen = Bitboard.set_nth tempboard.wqueen n_to}
    |(K, true) -> {tempboard with wking = Bitboard.set_nth tempboard.wking n_to; wcastle = false}
    |(P, false) -> {tempboard with bpawns = Bitboard.set_nth tempboard.bpawns n_to}
    |(B, false) -> {tempboard with bbishops = Bitboard.set_nth tempboard.bbishops n_to}
    |(N, false) -> {tempboard with bknights = Bitboard.set_nth tempboard.bknights n_to}
    |(R, false) -> {tempboard with brooks = Bitboard.set_nth tempboard.brooks n_to}
    |(Q, false) -> {tempboard with bqueen = Bitboard.set_nth tempboard.bqueen n_to}
    |_ -> {tempboard with bking = Bitboard.set_nth tempboard.bking n_to; bcastle = false}
  )
;;
